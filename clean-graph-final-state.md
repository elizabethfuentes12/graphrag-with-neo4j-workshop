...
gates in [`clean-graph.md`](clean-graph.md) are complete. The resulting changes
and artifacts are present in the repository working tree but have not been
committed or pushed.
...
- **Accepted replacement dump:**
  `setup/neo4j-hotel-graph-prebuilt.dump` and
  `static/neo4j-hotel-graph.dump` are both 6,542,982 bytes and have SHA-256
...
- **Regression protection:** The pinned offline setup suite passes 153 tests
  with one intentional environment-dependent skip. Repository integrity,
  shell syntax, Ruff lint and formatting, and whitespace validation also pass.
...
- **Version-control handoff:** Review, commit, and push the current working-tree
  changes when they are approved. No commit or remote publication has been
  performed yet.
- **Optional statistical benchmark:** Run the 240-trial evaluation only if the
  project needs comparative rates, stability estimates, or statistical claims.
  The completed 24-cell run is a release smoke for path coverage, not a
