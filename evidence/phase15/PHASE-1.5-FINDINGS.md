> **RETRACTED 2026-08-21. HISTORICAL EVALUATOR WARNING.** The conclusions
> below are withdrawn. The comparison used asymmetric prompts, graph-derived
> reference values, capped graph results, and questions that did not establish
> the claimed mechanism. The judge also received truncated evidence. Every
> historical grounding label and rate is invalid for publication until a
> repaired complete-evidence rescore exists. The deterministic observations may
> still be used to explain why the former module title was retired. The current
> decision and evaluator repair are recorded in `workfolder/defects-v2.md` and
...
any other. The raw JSON and generated detailed report are local-only diagnostics.
They have no durable URI and are not release evidence.
