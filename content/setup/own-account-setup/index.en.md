...
The `code-editor.yaml` template creates a VPC, Code Editor EC2 instance, and Neo4j on ECS Fargate with the hotel graph pre-loaded.

:::alert{type="warning" header="Before deploying"}
The hotel graph dump (`neo4j-hotel-graph.dump`) must be uploaded to the Workshop Studio S3 assets bucket first. If you are running self-paced without Workshop Studio, use the deploy command below with an S3 bucket you own.
:::
