...
- **Module 5 is already deployed and running in AWS**: the AgentCore Runtime, its ECR repository, and its
  CodeBuild project all exist, which is worth knowing because they keep accruing charges until someone
  removes them. Nothing in the workshop deletes them, so teardown is a manual step whenever you are done
  with the deployment.
- **What is left**:
  - Do one more walk through of all the notebooks.
  - Module 5's README.
  - Two open review findings on Module 5: a dependency list that pins every entry but one, and a build step
    that does not record what it packaged.
  - Live end-to-end runs.
  - **One decision for the repo author**: which of the two image folders is the real one. See section 5.
  - **One setup fix**: the install command creates `notebooks/.venv`, but nothing points the notebook kernel
    at it, so a participant following the README lands on the system Python and the first import fails.
    Registering the kernel, or pointing the README at it, closes the gap, and it is worth doing whichever way
    the images decision goes.
...
- **Nothing gets deleted**: whatever a participant builds stays, so there is no build-then-wipe step for an
...
- **Renumbered everything in one pass**: notebook names, notebook folders, content folders, page titles,
  page weights, image paths, and every cross-page link moved together. Moving all of them at once is what
  keeps the images and the links working.
- **Every name now matches the module it describes**: "Demo 01b", "Demo 06", "Module 7", "Module 8", and the
  old timing stamps are gone.
- **Every link now resolves**: six pointed at files and folders that no longer exist, and all six are gone.
- **The cleanup page folded into wrap-up**: hosted accounts get reclaimed after the event, so there is
  nothing for a participant to tear down. The parts worth keeping moved into the wrap-up page.
- **Filled in the summary page**: it now carries the workshop's argument and the decision tables, while
  wrap-up keeps the accomplishments, key concepts, and next steps. Each page now has its own job at the end
  of the day.
- **Added per-module READMEs**: five of six are written, and Module 5's is the last one to go.
...
- **The old `customer-service-*` names fell outside that grant**: an account running under the shipped policy
  would have stopped at the first role the notebook creates. Renamed and verified in `4.1`.
...
- **Every grounding check now has a positive control**: a grounded agent saying "I don't know" looks exactly
  like a broken one, so a test that checks only for the refusal keeps passing when nothing works. Each
  grounding check now sits beside an assertion on a real value, such as the Cairo hotel's full address, that
  a broken system could not produce.
...
- **They had drifted apart**: three slightly different programs sharing one set of names rather than backups
  of each other, so which one a notebook picked up depended on where it ran.
...
- **The `notebooks/` root holds no `.py` files at all**: this is the part that keeps the fix in place, because
  with nothing at the root there is no second copy to pick up by accident.
- **One implementation of each module**: the other two copies are gone.
- **Consolidating turned up two bugs worth finding early**: one that killed the reservation Lambda on
  startup, and a fixture file that had to be kept in three identical copies to be found at all. Both are
  fixed.
...
Two folders hold the same eleven image files:

- `static/images/`
- `workshop-content/images/`

I'm not sure which one we should keep?
